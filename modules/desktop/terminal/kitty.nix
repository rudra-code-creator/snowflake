...
with lib;
{
  ...
  config = mkIf (cfg.enable && (cfg.program == "kitty")) {
    hm.programs.kitty = {
      enable = true;
      settings = {
        term = "xterm-kitty";
      ...
      extraConfig = mkIf (activeTheme != null) ''
        include ~/.config/kitty/${activeTheme}.conf
      '';
    };
    ...
    create.configFile = mkMerge [
      {
        tab-bar = {
          ...
          source = "${config.snowflake.configDir}/kitty/powerline-bar.py";
        };
      }

      (mkIf (activeTheme != null) {
        kitty-theme = {
          target = "kitty/${activeTheme}.conf";
          text = with config.modules.themes; ''
            font_family               ${font.monospace.family} SemiBold
            italic_font               ${font.monospace.family} SemiBold Italic

            bold_font                 ${font.monospace.family} Bold
            bold_italic_font          ${font.monospace.family} Bold Italic

            font_size                 ${builtins.toString font.monospace.size}
