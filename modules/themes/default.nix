...
let
  ...
  session = config.modules.desktop.session;
in
with lib;
{
  options.modules.themes = with types; {
    ...
    loginWallpaper = my.mkOpt (nullOr path) (
      if cfg.wallpaper != null then
        my.toFilteredImage cfg.wallpaper "-gaussian-blur 0x2 -modulate 70 -level 5%"
      else
        null
    );
    ...
    font = {
      ...
      default = {
        family = my.mkOpt str "";
        weight = my.mkOpt str "Medium";
        weightNum = my.mkOpt int 500;
  ...
  config = mkIf (cfg.active != null) (mkMerge [
    ...
    (mkIf (session.type == "wayland") (mkMerge [
      (mkIf (cfg.wallpaper != null) {
