...
myConfig =
    def
        ...
        , focusedBorderColor = kanagawaBorder
        , logHook =
...
catpuccinBorder = "#F38BA8"
kanagawaBorder = "#938056"
miasmaBorder = "#BB7744"
rosepineBorder = "#EBBCBA"
tokyonightBorder = "#1ABC9C"
inactiveBorder = "#181825"

icyTheme =
    def
        { activeColor = kanagawaBorder
        , activeBorderColor = kanagawaBorder
        , activeTextColor = inactiveBorder
        , decoHeight = 20
        , inactiveColor = inactiveBorder
        , inactiveBorderColor = inactiveBorder
        , inactiveTextColor = kanagawaBorder
        , fontName = "xft:Maple Mono NF:size=13:style=Bold"
        }
...
zenBrowserSelector = appName =? "zen-beta" <&&> className =? "Navigator"

protonMailSelector = zenBrowserSelector <&&> fmap isProtonMailTitle title

virtualClasses =
    [(protonMailSelector, "Proton Mail"), (zenBrowserSelector, "zen-beta")]
...
-- :NOTE| Window wanagement through Hooks & namedScratchpads

emacsScratchID = "Emacs-NSP|"
emacsScratch = getInput $ inEditor >-> setFrameName emacsScratchID >-> eval (elispFun "dashboard-open")

emacsMailID = "Emacs-NSP: E-Mail"
emacsMailSP = getInput $ inEmacs >-> setTitleName emacsMailID >-> eval (elispFun "mu4e")

ircID = "Emacs-NSP: IRC"
ircSP = getInput $ inEmacs >-> setTitleName ircID >-> eval (elispFun "my/erc-start-or-switch")

matrixID = "Emacs-NSP: Ement"
matrixSP = getInput $ inEmacs >-> setTitleName matrixID >-> eval (elispFun "my/connect-to-matrix")

telegramID = "Emacs-NSP: Telega"
telegramSP = getInput $ inEmacs >-> setTitleName telegramID >-> eval (elispFun "telega")
...
myManageHook = composeAll
   [ title =? "System Monitor" --> floatCenter ]

myScratchpads :: X [NamedScratchpad]
myScratchpads = do
   emacsCommand <- emacsScratch
   emailCommand <- emacsMailSP
   ircCommand <- ircSP
   matrixCommand <- matrixSP
   telegramCommand <- telegramSP

   pure [ NS "Discord" "vesktop" (className =? "vesktop") floatCenter
        , NS "Emacs" emacsCommand (title ^? emacsScratchID) floatCenter
        , NS "IRC" ircCommand (title =? ircID) floatCenter
        , NS "Matrix" matrixCommand (title =? matrixID) floatCenter
        , NS "Mail" emailCommand (title =? emacsMailID) floatCenter
        , NS "Music" "youtube-music" (className =? "com.github.th_ch.youtube_music") floatCenter
        , NS "SoundCTL" "easyeffects" (title =? "Easy Effects") floatCenter
        , NS "Telegram" telegramCommand (title =? telegramID) floatCenter
...
addKeys conf@XConfig{modMask = modm} =
    ...
    buildDirectionalBindings modm myWindowGo
        ...
        ++ bindBringAndRaiseMany
            [ (modalt, xK_f, unsafeSpawn "zen-beta", zenBrowserSelector)
            , (modalt, xK_w, unsafeSpawn "zen-beta --private-window", zenBrowserSelector)
            ]
        -- Window manipulation
        ++ [ ((modm, xK_g), myGoToWindow)
           ...
           , -- :NOTE| Program-specific launches
             ...
             ((hyper, xK_s), runInTerm "--title='System Monitor'" "btop")
           ...
           , -- Playerctl
             ((modm, xK_Right), unsafeSpawn "playerctl next")
           , ((0, xF86XK_AudioNext), unsafeSpawn "playerctl next")
           , ((modm, xK_Left), unsafeSpawn "playerctl previous")
           , ((0, xF86XK_AudioPrev), unsafeSpawn "playerctl previous")
           , ((modm, xK_Down), unsafeSpawn "playerctl play-pause")
           , ((0, xF86XK_AudioPlay), unsafeSpawn "playerctl play")
           , ((0, xF86XK_AudioPause), unsafeSpawn "playerctl pause")
